...
### Runing pipelines locally
If you are familiar with docker and testcontainers api you can run pipelines on your machine.
See `RandomToConsolePipelineTest.java`, it already has configured local VoltSpContainer within test that will assert that pipeline has run and produced the result.
The test requires valid volt license path.
The test depnds on `master--latest` VoltSP docker image but you can set any image that is available to you.
Any other pipeline can be run in the same way, but you need to configure Kafka broker / VoltDB containers and start then.

### Runing pipelines in k8s
